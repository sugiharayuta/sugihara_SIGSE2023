...
\tabref{field_types}はレコードおよびクラスのインスタンスフィールド型の傾向を集計したものである．
...
\begin{table}[t]
    \caption{レコード（クラス）の実装インタフェースの傾向}
    \label{implemented_interfaces}
    \centering
    \scalebox{0.7}{
    \begin{tabular}{c||r|r||c||r|r}
        \hline
        ...
        ThrowingExternalizable & 26 & 3.0\% & Collector & 811 & 1.0\%\\
        ClusterStateTaskListener & 16 & 1.9\% & Writeable & 790 & 1.0\%\\
...
%         型名 & 個数 & 割合\\
%         \hline\hline
%         Serializable & 4,242 & 5.3\%\\
%         Runnable & 3,225 & 4.0\%\\
%         Iterator & 1,343 & 1.7\%\\
%         Comparable & 1,034 & 1.3\%\\
%         Cloneable & 888 & 1.1\%\\
%         Collector & 811 & 1.0\%\\
%         Writeable & 790 & 1.0\%\\
...
% \tabref{record_interface_types}はレコードの実装インタフェース，\tabref{class_interface_types}はクラスの実装インタフェースの傾向を集計したものである．
\tabref{implemented_interfaces}はレコードおよびクラスの実装インタフェースの傾向を集計したものである．
...
非レコード型（インタフェース等含む）からレコードへの変更695件のうち，クラスからレコードへ変更するリファクタリングは690件であった．
...
signal-cli，elasticsearch，casといったリポジトリでは，50件以上ものクラスをレコードに置き換えるリファクタリングのコミットが見られた．
